...
int main(int argc, char **argv) {
    ...
    int N;
    if (rank == 0) {
        printf("Ingrese el tamaño N de la matriz NxN (N > numero de procesos): ");
        scanf("%d", &N);
    }
    ...
    if (N <= size) {
        if (rank == 0) printf("Para parte b), N debe ser mayor que el numero de procesos\n");
        MPI_Finalize();
        return 1;
    }
